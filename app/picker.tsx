...
export default function Picker({ data }:{ data:Dataset }) {
  ...
  const [phase,setPhase] = useState<Phase>("station-ready");
  ...
  function spin(items:(Station|Restaurant)[]) {
    ...
    const show=(item:Station|Restaurant) => { current.current=item; last=item; setDisplay(name(item)); };
    show(order[index]);
  ...
  function stop(kind:"station"|"restaurant") {
    if (timer.current) clearInterval(timer.current);
    timer.current=null;
    const picked=current.current;
    if (!picked) return;
    setDisplay(name(picked));
    if (kind==="station") { setStation(picked as Station); setStationSearch(name(picked)); setPhase("restaurant-ready"); }
    else { setRestaurant(picked as Restaurant); setPhase("done"); }
  }
  ...
  function chooseStation(value:string) {
    ...
    setStation(picked); setRestaurant(null); setDisplay(name(picked)); setPhase("restaurant-ready");
  }
  ...
  function act() {
    ...
    else { setPhase("station-ready"); setStation(null); setRestaurant(null); setStationSearch(""); current.current=null; setDisplay(""); }
  }
  ...
  return <main className="shell">
    <header className="topbar">
      <div className="brand"><span className="brandmark">◎</span> MAKAN NEXT STOP</div>
      <button className="lang" onClick={()=>setLang(lang==="en"?"zh":"en")} aria-label="Switch language">{lang==="en"?"中文":"EN"}</button>
    </header>
    <section className="main">
      ...
      <div className="board">
        <section className="picker" aria-live="polite">
          <div className="step"><span>{t.step}</span><span>{phase.startsWith("station")?`01 — ${t.station}`:`02 — ${t.restaurant}`}</span></div>
          <div className={`rolling ${active?"active":""}`}>{shown}</div>
          <div>
            <label className="station-label" htmlFor="station-search">{t.choose}</label>
        ...
        <aside className="details">{restaurant ? <>
          <span className="tag">{station&&name(station)} · {restaurant.distanceM}m</span><h2>{name(restaurant)}</h2>
          <div className="info"><span>⌖</span><div><b>{t.address}</b>{restaurant.address}</div></div>
